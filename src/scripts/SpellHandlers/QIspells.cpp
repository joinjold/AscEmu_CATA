...
bool LayWreath(uint32 i, Spell* pSpell)  //Peace at Last quest
{
    ...
    if (pQuest == NULL || pQuest->GetMobCount(0) < pQuest->GetQuest()->ReqCreatureOrGOId[0])
        return true;
...
bool BalanceMustBePreserved(uint32 i, Aura* pAura, bool apply)
{
    ...
    if (lake1 != NULL)
    {
        if (qle->GetMobCount(0) < qle->GetQuest()->ReqCreatureOrGOId[0])
        {
    ...
    if (lake2 != NULL)
    {
        if (qle->GetMobCount(3) < qle->GetQuest()->ReqCreatureOrGOId[3])
        {
    ...
    if (lake3 != NULL)
    {
        if (qle->GetMobCount(1) < qle->GetQuest()->ReqCreatureOrGOId[1])
        {
    ...
    if (lake4 != NULL)
    {
        if (qle->GetMobCount(2) < qle->GetQuest()->ReqCreatureOrGOId[2])
        {
...
bool BuildingAPerimeter(uint32 i, Spell* pSpell)
{
    ...
    if (pQuest->GetMobCount(0) < pQuest->GetQuest()->ReqCreatureOrGOId[0])
    {
    ...
    if (pQuest->GetMobCount(1) < pQuest->GetQuest()->ReqCreatureOrGOId[1])
    {
    ...
    if (pQuest->GetMobCount(2) < pQuest->GetQuest()->ReqCreatureOrGOId[2])
    {
...
bool GoreBladder(uint32 i, Spell* pSpell)
{
    ...
    if (en && en->GetMobCount(0) < en->GetQuest()->ReqCreatureOrGOId[0])
    {
...
bool HunterTamingQuest(uint32 i, Aura* a, bool apply)
{
    ...
    else
    {
        ...
        Quest const* tamequest = objmgr.GetQuestTemplate(triggerspell->eff[1].EffectMiscValue);
        if (tamequest == NULL)
        {
        ...
        if (!p_caster->HasQuest(tamequest->GetQuestId()))
        {
        ...
        else if (!a->GetTimeLeft())
        {
            ...
            if (Rand(75.0f))    // 75% chance on success
            {
                ...
                if (m_target->IsCreature())
                {
                    ...
                    QuestLogEntry* qle = p_caster->GetQuestLogForEntry(tamequest->GetQuestId());
                    if (qle != NULL)
                    {
                        qle->SetMobCount(0, 1);
                        qle->SendUpdateAddKill(1);
...
bool CurativeAnimalSalve(uint32 i, Spell* pSpell) // Curing the Sick
{
    ...
    if (entry == 12296 || entry == 12298)
    {
        ...
        if (qle->GetQuest()->ReqCreatureOrGOId[0] == qle->GetMobCount(0))
            return true;
...
bool PoweringOurDefenses(uint32 i, Spell* pSpell)
{
    ...
    if (qle && qle->GetMobCount(0) < qle->GetQuest()->ReqCreatureOrGOId[0])
    {
        qle->SetMobCount(0, qle->GetMobCount(0) + 1);
        qle->SendUpdateAddKill(0);
        qle->UpdatePlayerFields();
...
bool ZethGorMustBurnHorde(uint32 i, Spell* pSpell)
{
    ...
    if (pQuest != NULL)
    {
        ...
        // Stable
        if (pQuest->GetMobCount(3) < pQuest->GetQuest()->ReqCreatureOrGOId[3])
        {
...
bool TheSeersRelic(uint32 i, Spell* pSpell)
{
    ...
    if (qle == NULL || qle->GetMobCount(0) >= qle->GetQuest()->ReqCreatureOrGOId[0])
        return true;
...
bool DisruptTheirReinforcements(uint32 i, Spell* pSpell)
{
    ...
    {
        bool SendMsg = false;
        if (pQuestA->GetMobCount(0) < pQuestA->GetQuest()->ReqCreatureOrGOId[0])
        {
        ...
        if (pQuestA->GetMobCount(1) < pQuestA->GetQuest()->ReqCreatureOrGOId[1])
        {
    ...
    {
        bool SendMsg = false;
        if (pQuestH->GetMobCount(0) < pQuestH->GetQuest()->ReqCreatureOrGOId[0])
        {
        ...
        if (pQuestH->GetMobCount(1) < pQuestH->GetQuest()->ReqCreatureOrGOId[1])
        {
...
bool LeyLine(uint32 i, Spell* pSpell)
{
    ...
    for (uint32 i = 0; i < sizeof(portals) / sizeof(uint32); i++)
    {
        ...
        if (portal != NULL && qle->GetMobCount(i) < qle->GetQuest()->ReqCreatureOrGOId[i])
        {
...
bool ReleaseUmisYeti(uint32 i, Spell* pSpell)
{
    ...
    {
        if (target->GetEntry() == friends[j] && qLogEntry->GetMobCount(j) < qLogEntry->GetQuest()->ReqCreatureOrGOId[j])
        {
...
bool CastFishingNet(uint32 i, Spell* pSpell)
{
    ...
    if (pPlayer->GetItemInterface()->GetItemCount(pQuest->GetQuest()->ReqItemId[0], true) < pQuest->GetQuest()->ReqItemCount[0])
