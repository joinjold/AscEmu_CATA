...
bool Living_Bomb(uint32 i, Aura* pAura, bool apply)
{
    ...
    if(caster && !apply)
        caster->CastSpell(pAura->GetTarget(), pAura->GetSpellProto()->eff[i].EffectBasePoints + 1, true);
    return true;
}
...
bool HotStreak(uint32 i, Aura* pAura, bool apply)
{
    if(i == 0)
    {
        ...
        if(apply)
        {
            ...
            caster->AddProcTriggerSpell(48108, pAura->GetSpellProto()->Id, caster->GetGUID(), pAura->GetSpellProto()->eff[i].EffectBasePoints + 1, PROC_ON_SPELL_CRIT_HIT | PROC_ON_SPELL_HIT, 0, pAura->GetSpellProto()->eff[i].EffectSpellGroupRelation, classMask);
        }
        else
...
bool MirrorImage(uint32 i, Aura* pAura, bool apply)
{
    ...
    if(caster != NULL && apply && i == 2)
        if(caster->GetGUID() == pAura->GetTarget()->GetCreatedByGUID())
            caster->CastSpell(pAura->GetTarget(), pAura->GetSpellProto()->eff[i].EffectTriggerSpell, true);
