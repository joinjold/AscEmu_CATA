...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param)
{
    ...
    Log.Debug("Guild", "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s"
        , session->GetPlayer()->GetNameString(), type, errCode, param.c_str());
}
...
void Guild::SendSaveEmblemResult(WorldSession* session, GuildEmblemError errCode)
{
    ...
    Log.Debug("Guild", "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u", session->GetPlayer()->GetNameString(), errCode);
}
...
void Guild::GuildEventLogEntry::SaveToDB() const
{
        ...
        CharacterDatabase.Execute("INSERT INTO guild_eventlog VALUES('%u', '%u', '%u', '%u', '%u', '%u', '%u')",
        m_guildId,
        m_guid,
        uint8(m_eventType),
        m_playerGuid1,
...
void Guild::GuildBankEventLogEntry::SaveToDB() const
{
        ...
        CharacterDatabase.Execute("INSERT INTO guild_bank_eventlog VALUES('%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')",
        m_guildId,
        m_guid,
        m_bankTabId,
        uint8(m_eventType),
...
{
        CharacterDatabase.Execute("DELETE FROM guild_newslog WHERE guildid = %u AND LogGuid = %u",
        m_guildId,
        GetGUID()
        );

        CharacterDatabase.Execute("INSERT INTO guild_newslog VALUES('%u', '%u', '%u', '%u', '%u', '%u', '%u')",
        m_guildId,
        GetGUID(),
        GetType(),
        GetPlayerGuid(),
...
void Guild::GuildRankInfo::SaveToDB(bool _delete) const
{
    if (_delete)
    {
        CharacterDatabase.Execute("DELETE FROM guild_rank WHERE guildid = %u AND rid = %u",
            m_guildId,
            m_rankId
            );
    }
...
void Guild::GuildRankInfo::CreateMissingTabsIfNeeded(uint8 tabs, bool _delete, bool logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i)
    {
        ...
        if (logOnCreate)
            Log.Error("Guild", "Guild %u has broken Tab %u for rank %u. Created default tab.", m_guildId, i, m_rankId);


            CharacterDatabase.Execute("DELETE FROM guild_bank_right WHERE guildid = %u AND TabId = %u",
            m_guildId,
            i
            );
            ...
            i,
            m_rankId,
            rightsAndSlots.GetRights(),
            rightsAndSlots.GetSlots()
            );
    }
}
...
void Guild::GuildRankInfo::SetName(std::string const& name)
{
    ...
    CharacterDatabase.Execute("UPDATE guild_rank SET rname = '%s', rid = %u WHERE guildid = %u",
        m_name.c_str(),
        m_rankId,
        m_guildId
        );
...
void Guild::GuildRankInfo::SetRights(uint32 rights)
{
    ...
    CharacterDatabase.Execute("UPDATE guild_rank SET rights = '%u' ,rid = '%u' WHERE guildid = %u",
        m_rights,
        m_rankId,
        m_guildId
        );
...
void Guild::GuildRankInfo::SetBankMoneyPerDay(uint32 money)
{
    ...
    CharacterDatabase.Execute("UPDATE guild_rank SET BankMoneyPerDay = '%u' ,rid = '%u' WHERE guildid = %u",
        money,
        m_rankId,
        m_guildId
        );
...
bool Guild::GuildBankTab::LoadItemFromDB(Field* fields)
{
    ...
    if (pItem == NULL)
    {
        CharacterDatabase.Execute("DELETE FROM guild_bankitems WHERE itemGuid = %u AND guildId = %u AND tabId = %u", fields[3].GetUInt32(), m_guildId, fields[1].GetUInt8());
    }
...
void Guild::GuildBankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    if (session)
    {
       Log.Debug("Guild", "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [%s]: Tabid: %u, Text: %s"
            , session->GetPlayer()->GetNameString(), m_tabId, m_text.c_str());
        session->SendPacket(&data);
    }
    else
    {
        Log.Debug("Guild", "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [Broadcast]: Tabid: %u, Text: %s", m_tabId, m_text.c_str());
...
bool Guild::GuildMember::LoadFromDB(Field* fields, Field* fields2)
{
    ...
    PlayerInfo* plr = objmgr.GetPlayerInfo((fields2[0].GetUInt32()));

    for (uint8 i = 0; i <= GUILD_BANK_MAX_TABS; ++i)
        m_bankWithdraw[i] = fields2[1 + i].GetUInt32();

    SetStats(plr->name,
        plr->lastLevel,                  // characters.level
        plr->cl,                         // characters.class
        plr->lastZone,                   // characters.zone
        plr->acct,                       // characters.account
        0);
...
void Guild::GuildMember::UpdateBankWithdrawValue(uint8 tabId, uint32 amount)
{
    ...
    CharacterDatabase.Execute("INSERT INTO guild_member_withdraw VALUES('%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u' ,'%u', '%u' )",
        Arcemu::Util::GUID_LOPART(m_guid),
        m_bankWithdraw[0],
        m_bankWithdraw[1],
        m_bankWithdraw[2],
...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    Log.Debug("Guild", "GUILD: creating guild [%s] for leader %s (%u)",
        name.c_str(), pLeader->GetNameString()->c_str(), Arcemu::Util::GUID_LOPART(m_leaderGuid));

    CharacterDatabase.Execute("DELETE FROM guild_member WHERE guildid = %u", m_id);

    

    CharacterDatabase.Execute("INSERT INTO guild (guildid, name, leaderguid, EmblemStyle, EmblemColor, BorderStyle, BorderColor, BackgroundColor, info, motd, createdate, BankMoney, level, experience, todayExperience) VALUES('%u', '%s', '%u', '%u', '%u', '%u', '%u', '%u', '%s', '%s', '%u', '%u', '%u', '0', '0')",
        m_id,
        name.c_str(),
        m_leaderGuid,
        m_emblemInfo.GetStyle(),
        m_emblemInfo.GetColor(),
        m_emblemInfo.GetBorderStyle(),
        m_emblemInfo.GetBorderColor(),
        m_emblemInfo.GetBackgroundColor(),
        m_info.c_str(),
        m_motd.c_str(),
        uint32(m_createdDate),
        m_bankMoney,
        _level
        );
...
void Guild::SaveToDB()
{
    CharacterDatabase.Execute("UPDATE guilds SET level = '%u', experience = '%u', todayExperience = '%u' WHERE guildId = %u",
        GetLevel(),
        GetExperience(),
        GetTodayExperience(),
        GetId()
...
void Guild::UpdateMemberData(Player* player, uint8 dataid, uint32 value)
{
    if (GuildMember* member = GetMember(player->GetGUID()))
    {
        ...
        case GUILD_MEMBER_DATA_LEVEL:
            ...
            break;
        default:
            Log.Error("Guild", "Guild::UpdateMemberData: Called with incorrect DATAID %u (value %u)", dataid, value);
            return;
        }
        //HandleRoster();
    }
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    if (session)
    {
        Log.Debug("Guild", "SMSG_GUILD_ROSTER [%s]", session->GetPlayer()->GetNameString());
        session->SendPacket(&data);
    }
    else
    {
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    Log.Debug("Guild", "SMSG_GUILD_QUERY_RESPONSE [%s]", session->GetPlayer()->GetNameString());
}
...
void Guild::SendGuildRankInfo(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    Log.Debug("Guild", "SMSG_GUILD_RANK [%s]", session->GetPlayer()->GetNameString());
}
...
void Guild::HandleSetMOTD(WorldSession* session, std::string const& motd)
{
    ...
    else
    {
        ...
        CharacterDatabase.Execute("UPDATE guild SET motd = '%s' WHERE guildId = %u",
        motd.c_str(),
        m_id
        );
...
void Guild::HandleSetInfo(WorldSession* session, std::string const& info)
{
    ...
    if (_HasRankRight(session->GetPlayer(), GR_RIGHT_MODIFY_GUILD_INFO))
    {
        ...
        CharacterDatabase.Execute("UPDATE guild SET info = '%s' WHERE guildId = %u",
            info.c_str(),
            m_id
            );
    }
...
void Guild::HandleSetBankTabInfo(WorldSession* session, uint8 tabId, std::string const& name, std::string const& icon)
{
    ...
    if (!tab)
    {
        Log.Error("Guild", "Guild::HandleSetBankTabInfo: Player %s trying to change bank tab info from unexisting tab %d.",
            session->GetPlayer()->GetNameString(), tabId);
        return;
    }
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    Log.Debug("Guild", "Player %s invited %s to join his Guild", player->GetNameString(), name.c_str());
...
void Guild::HandleAcceptMember(WorldSession* session)
{
    ...
    if (!sWorld.interfaction_guild &&      
        player->GetTeam() != leader->GetTeam())
        return;
...
void Guild::HandleRemoveRank(WorldSession* session, uint8 rankId)
{
    ...
    CharacterDatabase.Execute("DELETE FROM guild_bank_right WHERE rid = %u AND guildid = %u",
        rankId,
        m_id
        );

    // Delete rank
    CharacterDatabase.Execute("DELETE FROM guild_rank WHERE rid = %u AND guildid = %u",
        rankId,
        m_id
        );
...
void Guild::HandleGuildPartyRequest(WorldSession* session)
{
    ...
    if (!IsMember(player->GetGUID()) || !group)
        return;
    // TODO
    Log.Debug("Guild", "SMSG_GUILD_PARTY_STATE_RESPONSE [%s]", session->GetPlayer()->GetNameString());
}
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    Log.Debug("Guild", "SMSG_GUILD_EVENT_LOG_QUERY_RESULT [%s]", session->GetPlayer()->GetNameString());
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        data << uint32(tabId);
        session->SendPacket(&data);
        Log.Debug("Guild", "SMSG_GUILD_BANK_LOG_QUERY_RESULT [%s] TabId: %u", session->GetPlayer()->GetNameString(), tabId);
    }
}
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    Log.Debug("Guild", "SMSG_GUILD_PERMISSIONS_QUERY_RESULTS [%s] Rank: %u", session->GetPlayer()->GetNameString(), rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    data << int64(amount);
    session->SendPacket(&data);
    Log.Debug("Guild", "SMSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: %u", session->GetPlayer()->GetNameString(), amount);
}
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    Log.Debug("Guild", "SMSG_GUILD_EVENT [%s] MOTD", session->GetPlayer()->GetNameString());
...
bool Guild::LoadMemberFromDB(Field* fields, Field* fields2)
{
    ...
    {
        CharacterDatabase.Execute("DELETE FROM guild_member WHERE guildid = %u",
            lowguid
            );
        delete member;
        return false;
...
void Guild::_CreateNewBankTab()
{
    ...
    CharacterDatabase.Execute("DELETE FROM guild_bank_tab WHERE guildid = %u AND TabId = %u",
        m_id,
        tabId
        );

    CharacterDatabase.Execute("INSERT INTO guild_bank_tab VALUES('%u', '%u', '', '', '')",
        m_id,
        tabId
        );
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc)
{
    ...
    CharacterDatabase.Execute("DELETE FROM guild_rank WHERE guildid = %u",
        m_id
        );

    CharacterDatabase.Execute("DELETE FROM guild_bank_right WHERE guildid = %u",
        m_id
        );
...
bool Guild::_ModifyBankMoney(uint64 amount, bool add)
{
    ...
    CharacterDatabase.Execute("UPDATE guild SET BankMoney = '%u' WHERE guildid = '%u'",
        m_bankMoney,
        m_id
        );
...
void Guild::_SetLeaderGUID(GuildMember* pLeader)
{
    ...
    CharacterDatabase.Execute("UPDATE guild SET leaderguid = '%u' WHERE guildid = %u",
        Arcemu::Util::GUID_LOPART(m_leaderGuid),
        m_id
        );
}
...
void Guild::SendGuildReputationWeeklyCap(WorldSession* session, uint32 reputation) const
{
    ...
    data << uint32(cap);
    session->SendPacket(&data);
    Log.Debug("Guild", "SMSG_GUILD_REPUTATION_WEEKLY_CAP [%s]: Left: %u",
        session->GetPlayer()->GetNameString(), cap);
}
...
void Guild::HandleNewsSetSticky(WorldSession* session, uint32 newsId, bool sticky)
{
    ...
    if (itr == logs->end())
    {
        Log.Debug("Guild", "HandleNewsSetSticky: [%s] requested unknown newsId %u - Sticky: %u",
            session->GetPlayer()->GetNameString(), newsId, sticky);
        return;
    }
    ...
    Log.Debug("Guild", "HandleNewsSetSticky: [%s] chenged newsId %u sticky to %u",
        session->GetPlayer()->GetNameString(), newsId, sticky);
...
bool Guild::GuildBankTab::SetItem(uint8 slotId, Item* item)
{
    ...
    if (item)
    {
        CharacterDatabase.Execute("INSERT INTO guild_bank_items VALUES ('%u','%u','%u','%u')",
            m_guildId,
            m_tabId,
            slotId,
            item->GetLowGUID()
