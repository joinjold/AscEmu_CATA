...
void Player::smsg_TalentsInfo(bool SendPetTalents)
{
    ...
    {
        data << uint32(GetCurrentTalentPoints()); // Unspent talents
...
void Player::InitVisibleUpdateBits()
{
    ...
    for (uint16 i = PLAYER_QUEST_LOG_1_1; i < PLAYER_QUEST_LOG_25_2; i += 5)
...
void Player::LearnTalent(uint32 talentid, uint32 rank, bool isPreviewed)
{
    ...
    TalentTabEntry *talentTabInfo = dbcTalentTab.LookupEntry(talentInfo->TalentTree);

    if (!talentTabInfo)
    {
        sLog.outDebug("Could not find talent tab %u in talent DBC \n", talentInfo->TalentTree);
        return;
    }
    ...
    else
    {
        ...
        if (!HasSpell(spellid))
        {
            ...
            if (getClass() == HUNTER && GetSummon())
                GetSummon()->RemoveAuraByNameHash(spellInfo->NameHash);
...
{
    if (opcode == CMSG_MOVE_FALL_LAND || minfo.GetMovementFlags() & MOVEMENTFLAG_SWIMMING)
    {
        jumping = false;
        return false;
    }
...
void Player::HandleBreathing(MovementInfo & movement_info, WorldSession* pSession)
{
    ...
    if (!sWorld.BreathingEnabled || FlyCheat || m_bUnlimitedBreath || !isAlive() || GodModeCheat)
    {
        ...
        if (m_UnderwaterState & UNDERWATERSTATE_UNDERWATER)
        {
            ...
            data << uint32(TIMER_BREATH) << m_UnderwaterTime << m_UnderwaterMaxTime << int32(-1) << uint32(0);
    ...
    if (movement_info.GetMovementFlags() & MOVEMENTFLAG_SWIMMING && !(m_UnderwaterState & UNDERWATERSTATE_SWIMMING))
    {
    ...
    if (m_UnderwaterState & UNDERWATERSTATE_SWIMMING && m_UnderwaterState & UNDERWATERSTATE_UNDERWATER)
    {
        ...
        if ((movement_info.pos.m_positionZ + m_noseLevel) > pSession->m_wLevel)
        {
            m_UnderwaterState &= ~UNDERWATERSTATE_UNDERWATER;
            WorldPacket data(SMSG_START_MIRROR_TIMER, 20);
            data << uint32(TIMER_BREATH) << m_UnderwaterTime << m_UnderwaterMaxTime << int32(10) << uint32(0);
            pSession->SendPacket(&data);
        }
    }
...
bool Player::IsSpellFitByClassAndRace(uint32 spell_id)
{
